...
namespace App\Filament\Resources\RentalListings\Pages;

use App\Filament\Resources\RentalListings\RentalListingsResource;
...
class ListRentalListings extends Page implements HasTable
{
    use InteractsWithTable;
    
    protected static string $resource = RentalListingsResource::class;
    protected string $view = 'filament.resources.rental-listings.pages.list-rental-listings';
    ...
    protected function getHeaderActions(): array
    {
        return [
            Action::make('clear_cache')
                ...
                ->action(function () {
                    \Illuminate\Support\Facades\Cache::forget('goyzer_rental_listings');
                    Notification::make()
    ...
    {
        return $table
            ->records(fn (?string $search = null, ?array $filters = null, ?int $page = null, ?int $recordsPerPage = null): LengthAwarePaginator => $this->getRentalListingsData($search, $filters ?? [], $page ?? 1, $recordsPerPage ?? 10))
            ->columns([
                ...
                TextColumn::make('RentPerYear')->label('Rent/Year')->money('AED')->sortable(),
            ...
            ->filters([
                ...
                \Filament\Tables\Filters\Filter::make('RentPerYear')
                    ->form([
                        \Filament\Forms\Components\TextInput::make('rent_from')->numeric(),
                        \Filament\Forms\Components\TextInput::make('rent_to')->numeric(),
                    ]),
            ])
            ->searchable()
    ...
    protected function getRentalListingsData(?string $search = null, array $filters = [], int $page = 1, int $recordsPerPage = 10): LengthAwarePaginator
    {
        $result = \Illuminate\Support\Facades\Cache::rememberForever('goyzer_rental_listings', function () {
            return app(GoyzerService::class)->getRentalListings();
        });
        ...
        if (filled($filters['RentPerYear']['rent_from'] ?? null)) {
            $collection = $collection->filter(fn($listing) => ($listing['RentPerYear'] ?? 0) >= $filters['RentPerYear']['rent_from']);
        }

        if (filled($filters['RentPerYear']['rent_to'] ?? null)) {
            $collection = $collection->filter(fn($listing) => ($listing['RentPerYear'] ?? 0) <= $filters['RentPerYear']['rent_to']);
        }
